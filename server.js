...
// Rate limiting for API endpoints (excluding health and root)
app.use((req, res, next) => {
  if (req.path === '/health' || req.path === '/') {
    return next();
  }
  return createRateLimiter(60000, 100)(req, res, next); // 100 requests per minute
});
...
⏰ Detected: ${new Date(opportunity.timestamp).toLocaleString()}`;
  ...
  async analyzeWithTechnicalIndicators(coin, context = {}) {
    ...
    try {
      ...
      // Extract values for signal evaluation
...
// API routes
app.post('/start-scan', async (req, res, next) => {
  try {
...
app.post('/stop-scan', (req, res, next) => {
  try {
...
app.post('/scan-now', async (req, res, next) => {
  try {
...
app.post('/test-telegram', async (req, res, next) => {
  try {
...
app.get('/live-analysis', (req, res, next) => {
  try {
...
app.get('/scan-history', (req, res, next) => {
  try {
...
app.post('/auto-scan-settings', (req, res, next) => {
  try {
    const { interval } = req.body || {};
    if (!interval) {
      const err = new Error('Interval is required');
      err.status = 400;
      throw err;
    }
...
app.get('/scan-progress', (req, res, next) => {
  try {
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`\n🚀 Professional Crypto Scanner V2`);
  ...
  console.log(`📰 News: ${NEWS_ENABLED ? 'ENABLED ✅' : 'DISABLED ⚠️'}`);
